...
const runSSMCommand = async (inputs) => {
    ...
    try {
        ...
        if (inputs.waitForCompletion && COMMAND_ID) {
            const RESULT = await waitForCommandCompletion(SSM_CLIENT, COMMAND_ID, inputs.instanceIds[0], inputs.pollingIntervalSeconds, inputs.timeoutSeconds);
            if (inputs.haveConsoleLog) {
        ...
        throw new Error(`Failed to send or execute SSM command: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
};
const waitForCommandCompletion = async (client, commandId, instanceId, pollingIntervalSeconds, timeoutSeconds) => {
    const START_TIME = Date.now();
    const END_TIME = START_TIME + timeoutSeconds * 1000;
    // eslint-disable-next-line no-loops/no-loops
    while (Date.now() < END_TIME) {
        const INVOCATION_RESULT = await client.send(new GetCommandInvocationCommand({
            CommandId: commandId,
